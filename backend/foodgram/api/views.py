...
def shoppingcart_detail(request, id):
    ...
    if request.method == "POST":
        if ShoppingCart.objects.filter(owner=request.user, recipe=recipe).exists():
            return Response(
                "Рецепт уже находиться в корзине", status=status.HTTP_400_BAD_REQUEST
            )
...
def download_shopping_cart(request):
    if request.method == "GET":
        ...
        for ingredient_name, amount in ingredients_count.items():
            ...
            measurement_unit = (
                ingredient.ingredient.measurement_unit if ingredient else "единица"
            )
...
def favorite_detail(request, id):
    ...
    if request.method == "POST":
        if FavoriteRecipe.objects.filter(user=request.user, recipe=recipe).exists():
            return Response(
                "Рецепт уже находиться в избранном", status=status.HTTP_400_BAD_REQUEST
            )
...
def subscribe_detail(request, id):
    ...
    subscribe.delete()
    return Response(
        f"Вы отписались от {author.username}", status=status.HTTP_204_NO_CONTENT
    )
